...
#define TRACE(...)                 \
  do {                             \
    if (1) {                       \
      PrintF("vector promotion:"); \
      PrintF(__VA_ARGS__);         \
    }                              \
  } while (false)

VectorPromotion::VectorPromotion(MachineGraph* mcgraph,
...
void VectorPromotion::PromoteGraph() {
...
void VectorPromotion::ReplaceSimdNodes() {
...
void VectorPromotion::PromoteLoadNode(Node* node) {
...
void VectorPromotion::PromoteStoreNode(Node* node) {
  ...
  switch (node->opcode()) {
    case IrOpcode::kStore: {
      ...
      store_op = machine()->Store(StoreRepresentation(rep, write_barrier_kind));
      break;
    }
...
void VectorPromotion::PromoteLoadTransformNode(Node* node) {
  const Operator* op;
...
void VectorPromotion::PromoteNode(Node* node) {
  const Operator* op;
  ...
  switch (node->opcode()) {
    ...
    case IrOpcode::kUnalignedLoad:
    case IrOpcode::kLoadImmutable:
      PromoteLoadNode(node);
      break;

    case IrOpcode::kStore:
    case IrOpcode::kUnalignedStore:
    case IrOpcode::kProtectedStore:
      PromoteStoreNode(node);
      break;

    case IrOpcode::kLoadTransform:
      PromoteLoadTransformNode(node);
      break;
